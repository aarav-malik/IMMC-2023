...
benefits = pd.read_csv('data/data for SB/Socialbenefitsdata.csv', delimiter =',')
salarys = pd.read_csv('data/data for SB/Salary benefits data.csv', delimiter =',')
...
salary_magnitude = pd.read_csv('data/data for SB/Normalised salary magnitude.csv', delimiter =',')
...
for x in final_table.iterrows():
    normalised_socialbenefits = 1 + ((final_table['Overall score']-np.nanmin(final_table['Overall score']))*(2-1))/(np.nanmax(final_table['Overall score'])-np.nanmin(final_table['Overall score']))
